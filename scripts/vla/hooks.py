#!/usr/bin/env python
"""Hook / patching harness for LeRobot pi0.5 (docs/vla-stage2-protocol-2026-08-30.md §1, §4.1, §9.1).
...
class Pi05Harness:
    ...
    def prefix_forward(self, batch, resid_hooks=None, capture=False, prefix_embs_override=None, attn_hook=None):
        ...
        prefix_embs_override, when provided, replaces the embedded prefix before layer 0 while preserving the
        destination batch's masks and positions. This is used for prefill-time causal interventions.
        attn_hook, when provided, is called after eager attention and may return a replacement ``(output,
        weights)`` pair. It receives the layer index, raw Q/K/V tensors, mask/scaling arguments, the original
        output/weights, and the unwrapped eager-attention function. This narrow interface supports attention-edge
        interventions without replacing a complete layer residual."""
        m = self.policy.model
        prefix_embs, prefix_pad_masks, prefix_att_masks, n_img_slots, n_img_valid = self._prefix_embed(batch)
        if prefix_embs_override is not None:
            if prefix_embs_override.shape != prefix_embs.shape:
                raise AssertionError(
                    f"prefix override shape {prefix_embs_override.shape} != {prefix_embs.shape}"
                )
            prefix_embs = prefix_embs_override.to(device=prefix_embs.device, dtype=prefix_embs.dtype)
        ...
        if attn_hook is not None:
            ...
            attn_mod2layer = {id(layer.self_attn): index for index, layer in enumerate(self.vlm_layers)}
            ...
            def wrapped(module, query, key, value, attention_mask, scaling, dropout=0.0, **kw):
                ...
                layer_index = attn_mod2layer.get(id(module))
                if layer_index is None:
                    return out, weights
                replacement = attn_hook(
                    ...
                    output=out,
                    weights=weights,
                    original_forward=orig,
                    forward_kwargs=kw,
                )
                return (out, weights) if replacement is None else replacement
    ...
    def action_forward(self, prefix, noise, expert_hooks=None, capture=False, num_steps=None, attend_mask=None,
                       attn_segments=None, attn_hook=None):
        ...
        for L, layer in enumerate(self.expert_layers):
            handles.append(layer.register_forward_hook(mk(L)))
        attn_patch = None
        if attn_segments is not None or attn_hook is not None:
            from transformers.models.gemma import modeling_gemma as mg
            seg_names = list(attn_segments or {})
            ...
            def wrapped(module, query, key, value, attention_mask, scaling, dropout=0.0, **kw):
                ...
                if id(module) in attn_mod2layer and w is not None and w.shape[-2] == self.cfg.chunk_size:
                    ...
                    if seg_names:
                        nonlocal mass
                        if mass is None:
                            mass = torch.zeros(len(self.expert_layers), n_steps, w.shape[1], len(seg_names))
