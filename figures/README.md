...
**Three aligned panels, five source tests:** panel a is 10-way grouped-cross-validation probe accuracy on 300 Goal confirmation units. Panel b shows the marginal fraction of the full action effect produced by adding one layer to a cumulative instruction→image attention-message block, from both ends of the stack; this is a causal development profile, **not raw activation change**, and its preregistered classification was ambiguous because neither peak reached the localized threshold. The annotation reports the separate 12-cell L6→8 attention+MLP ablation and failed eight-endpoint compact-writer test. Panel c plots all six directed prompt-pair cells and their median at the actual six-layer K/V-intervention resolution, then annotates the separate 20-rollout early and late behavioral outcomes. Connecting a cell's three measured bands aids reading; it does not imply an unmeasured single-layer K/V sweep.
