...
def main():
    ...
    ap.add_argument("--cells_json", default=None,
                    help='JSON list of [cellname, task_id, whichA, tB] overriding CELLS. '
                         'Used to point the sweep at the libero_goal cells where Stage 2 localized '
                         'the channel (the object-suite Stage 2 is weak, so steering there was '
                         'testing the intervention off-site).')
    ...
    if not lam0_ok:
        if not base_repeatable:
            ...
            # GATE RESPEC: bitwise reproduction of a ~135-step bf16 closed-loop rollout is
            # unachievable in principle. The edit allocates fp32 temporaries inside the prefix pass, which
            # moves the CUDA allocator and perturbs the bf16 GEMMs of the action expert at the ULP level.
            # The prefix KV cache itself IS bitwise unchanged at lambda=0 (verified separately), so this is
            # a numerics floor, not a content error. The gate that matters is behavioural: lambda=0 must
            # give the SAME success and the SAME first-touched object as the unedited rollout.
            beh_ok = all(s["plain_vs_lam0"].get("same_success") and s["plain_vs_lam0"].get("same_first_touch")
                         for s in sanity)
            worst = max(s["plain_vs_lam0"].get("max_abs_eef_diff", 0.0) for s in sanity)
            if beh_ok:
                print("[pilot] SANITY (behavioural) PASSED: lambda=0 is not bitwise "
                      "(max abs eef diff = %.4g) but success and first_touch match unedited on every sanity "
                      "init. Proceeding; this noise floor is reported alongside the lambda effects." % worst,
                      flush=True)
                manifest["sanity_gate"] = {"mode": "behavioural", "max_abs_eef_diff": worst}
            else:
                print("[pilot] FATAL: lambda=0 changes SUCCESS or FIRST-TOUCH vs unedited -> real harness "
                      "bug, stopping.", flush=True)
                sys.exit(3)
    else:
    ...
    if args.cells_json:
        import json as _j
        _cells = [tuple(c) for c in _j.load(open(args.cells_json))]
        print(f"[pilot] using {len(_cells)} cells from {args.cells_json}", flush=True)
    for cellname, tid, whichA, tB in _cells:
        env = make_env(suite, args.suite, tid)
        tracker = ContactTracker(env._env)
