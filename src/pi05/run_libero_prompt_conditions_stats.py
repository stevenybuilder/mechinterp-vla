# COPY of run_libero_prompt_conditions.py with --stats_json/--norm_mode/--dtype for checkpoints that ship no normalizer stats (pi05_libero_base, pi05_base).
#!/usr/bin/env python
...
def build_stats(stats_json):
    """openpi norm_stats.json -> lerobot dataset_stats dict (observation.state / action)."""
    j = json.load(open(stats_json))["norm_stats"]
    return {"observation.state": {k: j["state"][k] for k in ("mean", "std", "q01", "q99") if k in j["state"]},
            "action": {k: j["actions"][k] for k in ("mean", "std", "q01", "q99") if k in j["actions"]}}


def load_policy(policy_path, revision, n_action_steps, device, stats_json=None, norm_mode="MEAN_STD", dtype=None):
    if revision:
    ...
    policy.eval()
    pre_over = {"device_processor": {"device": device}, "rename_observations_processor": {"rename_map": {}}}
    post_over = {}
    if stats_json:
        stats = build_stats(stats_json)
        feats = {k: {"type": v.type.value, "shape": list(v.shape)} for k, v in {**pcfg.input_features, **pcfg.output_features}.items()}
        norm_map = {"VISUAL": "IDENTITY", "STATE": norm_mode, "ACTION": norm_mode}
        pre_over["normalizer_processor"] = {"stats": stats, "features": feats, "norm_map": norm_map}
        post_over["unnormalizer_processor"] = {"stats": stats, "features": {"action": feats["action"]}, "norm_map": norm_map}
        print(f"[stats] injected {stats_json} norm_mode={norm_mode} features={list(feats)}", flush=True)
    pre, post = make_pre_post_processors(policy_cfg=pcfg, pretrained_path=policy_path,
                                         preprocessor_overrides=pre_over, postprocessor_overrides=post_over)
    if stats_json:
        ns = [s for s in pre.steps if s.__class__.__name__.startswith("Normalizer")]
        assert ns and ns[0]._tensor_stats, "normalizer stats NOT injected"
        print(f"[stats] normalizer keys={list(ns[0]._tensor_stats)} unnormalizer ok", flush=True)
...
def main():
    ...
    ap.add_argument("--stats_json", default=None, help="openpi norm_stats.json to inject (checkpoints without shipped stats)")
    ap.add_argument("--norm_mode", default="MEAN_STD", choices=["MEAN_STD", "QUANTILES"])
    ap.add_argument("--dtype", default=None, help="override policy dtype, e.g. bfloat16")
    args = ap.parse_args()
    ...
    policy, pcfg, pre, post, epre, epost = load_policy(args.policy, args.revision, args.n_action_steps, args.device, args.stats_json, args.norm_mode, args.dtype)
    ...
    print(f"[runner] suite={args.suite} tasks={task_ids} inits={init_ids} conds={conditions} out={out}", flush=True)
        ...
        for init_id in init_ids:
            for cond in conditions:
                ...
                rec.update({
                    ...
                    "target_object": target_obj, "wrong_task_id": wrong_tid if cond == "wrong_object" else None,
                    ...
                    "n_action_steps": args.n_action_steps, "stats_json": args.stats_json, "norm_mode": args.norm_mode if args.stats_json else None, "dtype": args.dtype, "max_steps": max_steps, "tag": args.tag,
