#!/usr/bin/env python
"""OpenVLA-OFT Stage 2 harness (plan docs/protocol.md §A; protocol §9.1 OFT column).
...
class OFTAdapter:
    ...
    def forward(self, b, layer_hooks=None, attn_hook=None, stop_id=None):
...
def main():
    ...
    for tid in task_ids:
        ...
        cands = [j for j in range(suite.n_tasks) if j not in (tid, b_tid) and GOAL_TARGET_OBJECT[j] not in (objA, objB)]
        ...
        for init_id in range(args.n_init):
            ...
            for tp in timepoints:
                ...
                for src, dst in (("B", "A"), ("A", "B")):
                    for site in ("INSTR", "IMG", "ACT", "PROPRIO"):
