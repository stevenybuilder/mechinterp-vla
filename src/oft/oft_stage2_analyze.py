...
def probe(features, labels, groups, n_null=200, seed=0):
    ...
    # PCA-64 fitted ONCE on all items (unsupervised, label-free; a mild transductive step shared by real and null fits)
    features = PCA(min(64, features.shape[0] - 1), random_state=0).fit_transform(StandardScaler().fit_transform(features))

    def cv_acc(y):
        accs = []
        for tr, te in GroupKFold(min(5, len(np.unique(groups)))).split(features, y, groups):
...
def main():
    ...
    labels = np.array([man["tasks"][k[0]][k[3]] for k in keys])
    ...
    for L in ([int(x) for x in args.probe_layers.split(",")] if len(np.unique(groups)) >= 2 else []):
        row = {}
